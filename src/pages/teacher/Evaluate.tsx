...
const Evaluate = () => {
  ...
  const { classes, user } = useAuthStore();
  ...
  const getTeacherEvaluation = (sub) => {
    if (!sub) return { grade: '', feedback: '' };
    const myTeacherId = user?.id || user?.userId;
    if (sub.evaluations && sub.evaluations.length > 0) {
      const myEval = sub.evaluations.find(e => e.teacherId === myTeacherId);
      if (myEval) return { grade: myEval.grade?.toString() || '', feedback: myEval.feedback || '' };
      return { grade: '', feedback: '' };
    }
    // Geriye dönük uyumluluk (legacy)
    if (sub.status === 'graded' && sub.gradedBy === myTeacherId) {
      return { grade: sub.grade?.toString() || '', feedback: sub.feedback || '' };
    }
    return { grade: '', feedback: '' };
  };

  const isGradedByMe = (sub) => {
    if (!sub) return false;
    const myTeacherId = user?.id || user?.userId;
    if (sub.evaluations && sub.evaluations.length > 0) {
      return sub.evaluations.some(e => e.teacherId === myTeacherId);
    }
    if (sub.status === 'graded') {
      // Sadece bu öğretmenin değerlendirdiği legacy kayıtları göster
      // Not: gradedBy alanı eski verilerde teacherId'yi tutuyor
      return sub.gradedBy === myTeacherId;
    }
    return false;
  };
        ...
        if (submissionId) {
          ...
          if (sub) {
            ...
            if (!currentSubmission || currentSubmission.studentId !== sub.studentId) {
              ...
              const evalData = getTeacherEvaluation(sub);
              setGrade(evalData.grade);
          ...
          if (!currentSubmission) {
            ...
            const evalData = getTeacherEvaluation(groupedSubs[0]);
            setGrade(evalData.grade);
            setFeedback(evalData.feedback);
  ...
  const filteredSubmissions = examSubmissions.filter(s => {
    ...
    const matchesStatus = filterStatus === 'all' ||
                         (filterStatus === 'graded' && isGradedByMe(s)) ||
                         (filterStatus === 'ungraded' && !isGradedByMe(s));
  ...
  const handleSelectSubmission = (submission, index) => {
    ...
    const evalData = getTeacherEvaluation(submission);
    setGrade(evalData.grade);
    setFeedback(evalData.feedback);
  ...
  const handleSaveGrade = async (notify = false) => {
    ...
    try {
      ...
      const updateSubmission = (sub) => {
        ...
        const idx = evals.findIndex(e => e.teacherId === myTeacherId);
        if (idx >= 0) evals[idx] = newEval;
      ...
      const updatedSubs = examSubmissions.map(s => 
        s.id === currentSubmission.id ? updateSubmission(s) : s
      );
      setExamSubmissions(updatedSubs);
      setCurrentSubmission(updateSubmission(currentSubmission));
  ...
  const gradedCount = examSubmissions.filter(s => isGradedByMe(s)).length;
  ...
  if (!examId) {
    ...
    const allExams = (exams || []).filter(exam => {
      // Sadece kendi oluşturduğumuz sınavları görelim
      if (exam.createdBy && exam.createdBy !== myTeacherId) {
        return false;
      }
          ...
          {allExams.length === 0 ? (
            ...
            <div style={{ display: 'grid', gap: '16px' }}>
              {allExams.map(exam => {
                ...
                const gradedCount = new Set(examSubs.filter(s => isGradedByMe(s)).map(s => s.studentId)).size;
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '300px' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <button 
              ...
              onClick={() => navigate('/ogretmen/degerlendirme')}
            >
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      {isGradedByMe(submission) ? (
                        <div style={{ textAlign: 'center' }}>
                          <p style={styles.gradeDisplay}>{getTeacherEvaluation(submission).grade}</p>
