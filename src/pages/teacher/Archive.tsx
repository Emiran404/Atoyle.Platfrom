...
const Archive = () => {
  ...
  // Öğrencisi olan sınıfları bul (büyük/küçük harf duyarsızlaştırma)
  const classesWithStudents = new Set(
    students?.map(s => ((s as any).class || (s as any).className || '').trim().toUpperCase()).filter(Boolean) || []
  );
  
  // Available classes for dropdown (ve istatistik). Sadece öğrencisi olan sınıflar aktif sayılır.
  const availableClasses = classes
    .map(cls => typeof cls === 'string' ? cls : cls.name)
    .filter(cls => classesWithStudents.has((cls || '').trim().toUpperCase()))
    .sort();
